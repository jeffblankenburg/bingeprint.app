-- Drop the most-watched-actors leaderboard. TMDB's per-episode cast is too
-- sparse to make it accurate (a mainstream show like Parks & Rec returns no
-- episode cast for ~55% of episodes; niche shows are worse), and a show-level
-- approximation isn't a number we want to stand behind. Insights keeps only
-- what's exact: counts, watch time, genres, and eras.
